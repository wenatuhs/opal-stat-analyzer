# Python peak detector, see https://gist.github.com/1178136
# Adapated by Andreas Adelmann, Yves Ineichen
...
def getMyMaxPeaks(ydata, xdata, lookahead, proof):
    _max1, _min1 = peakdetect(ydata, xdata, lookahead, proof)
    xmax = [p[0] for p in _max1]
    ymax = [p[1] for p in _max1]
    return xmax, ymax

def getMyMinPeaks(ydata, xdata, lookahead, proof):
    _max1, _min1 = peakdetect(ydata, xdata, lookahead, proof)
    xmin = [p[0] for p in _min1]
    ymin = [p[1] for p in _min1]
    return xmin, ymin

def applyVeto(xdata, ydata, vetoList):
    # Filter out unwanted data
    for k in range(0, len(vetoList), 2):
        xmin = float(vetoList[k])
        xmax = float(vetoList[k+1])
        i = 0
        for xval in nditer(xdata):
...
def calcDistanceToClosestPeak(xdata, ydata, peak_at):
    idx = 0
    if (len(xm1) != 1):
        xnp = np.array(xdata)
        idx = (np.abs(xnp - peak_at)).argmin()

    fn = dataCol + '-peak.dat'
    myfile = open(fn, 'w')
    myfile.write(str(xdata[idx]))
    myfile.close()

    return math.fabs(peak_at - xdata[idx])


from sddsParser import SddsParser
...
if __name__ == "__main__":
    ...
    for arg in sys.argv:
        ...
        if arg.startswith("--findMinimum"):
            findMin = True

        if arg.startswith("--lookAhead"):
            look_ahead = str.split(arg, "=")[1]

        if arg.startswith("--simdata"):
            simFn = str.split(arg, "=")[1]

        if arg.startswith("--datacol"):
            dataCol = str.split(arg, "=")[1]

        if arg.startswith("--peakrange"):
            vetoStr =  str.split(arg, "=")[1]
            vetoData = str.split(vetoStr, ",")
            if len(vetoData) % 2:
                print "Veto data must be even in lengt size= ", len(vetoData)

        if arg.startswith("--peakAt"):
            peakAt = str.split(arg, "=")[1]


    sdds_file = SddsParser(simFn)
    x = np.array(sdds_file.getColumn(dataCol))
    s = np.array(sdds_file.getColumn("s"))

    if not (vetoData == ""):
        x = applyVeto(s, x, vetoData)

    if findMin:
        xm1, ym1 = getMyMinPeaks(x, s, look_ahead, 0)
    else:
        xm1, ym1 = getMyMaxPeaks(x, s, look_ahead, 0)

    sErrSQ = float(peakAt)
    if len(xm1) != 0:
        sErrSQ = calcDistanceToClosestPeak(xm1, ym1, float(peakAt))

    print sErrSQ

    fn = dataCol + '-err.dat'
    myfile = open(fn, 'w')
    myfile.write(str(sErrSQ))
    myfile.close()
